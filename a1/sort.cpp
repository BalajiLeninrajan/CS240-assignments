...
private:
  ...
  // generates inverse permutations
  ...
  // calls sorting function on all permutations to generate decision tree
  void generate_tree() {
...
void insertion_sort(DecisionTree &dtree, vector<int> &A,
  ...
  for (int i = 1; i < n; i++)
    for (int j = i; j > 0; j--) {
      dtree.dtree_add(Node{Alookup[A[j - 1]], Alookup[A[j]], -1},
                      dt_idx); // add node to tree
      if (A[j - 1] > A[j]) {
...
void selection_sort(DecisionTree &dtree, vector<int> &A,
  ...
  for (int i = 0; i < n - 1; i++) {
    int idx = i;
    for (int j = i + 1; j < n; j++) {
      dtree.dtree_add(Node{Alookup[A[idx]], Alookup[A[j]], -1},
                      dt_idx); // add node to tree
      if (A[j] < A[idx]) {
        dt_idx = 2 * dt_idx + 1; // left child
        idx = j;
...
int main(int argc, char *argv[]) {
  ...
  DecisionTree dt(algo_select, algo_n); // initialise tree
  dt.print_tree();
  return 0;
}
